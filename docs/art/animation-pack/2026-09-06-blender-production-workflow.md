...
In player-eye view, inspect nearby pressure, distant teammates, sticks, goalie coverage and puck visibility at actual desktop and narrow-screen sizes. Own head/body is hidden to prevent camera clipping in the current implementation; an own-glove/stick foreground model is not delivered. Player-eye view uses age-dependent eye height, full boards and a neutral 3D enclosure with walls, stands, roof and beams. This replaces the dark void seen in the earlier phone review, but is not Sense Arena art parity. The current eye camera is anchored to the actor root with independent look direction; it does not track the deformed eye socket or automatically follow authored head scans.
