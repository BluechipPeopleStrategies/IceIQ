...
Moshey owns visual craft, including Blender/GLB, geometry, materials and motion. Assign a task-specific RinkReads scope; Hockey Authority owns the hockey requirements and domain review, and Reel owns independent viewer clarity. The interactive application remains responsible for question entry, camera manipulation, playback, answers and source-state preservation. Do not import BlueChip's branding, vertical-video constraints or publishing workflow into this product.

Source correction, September 6: this paragraph previously described Moshey as a Remotion/transparent-overlay specialist. The accepted September 6 editorial/craft split in `C:/Users/mtsli/BlueChip/.claude/agents/moshey.md` supersedes that description. This corrects the role reference, not BlueChip policy.
