...
Preserve the approved six-world artwork and navy/gold direction. Standardize markers, pills and containers; test wrapping and spacing at narrow widths. Interactive movement serves the lesson: pan/zoom/reset, accessible alternatives and clear labels. Three-dimensional rink/referee samples and detailed multi-view helmet illustrations are available; catalog-wide 3D and polished production characters remain later work. Avoid autoplay as a requirement. Respect reduced motion and preserve 2D fallback.
...
September 18 artwork direction: automatically use the built-in ChatGPT Image tool for new representational equipment/object artwork. Prefer recognizable, literal objects (including an actual open hockey bag) over abstract placeholder icons. Retain useful text labels, pan/zoom and keyboard alternatives. Generated images are illustrations, not product photographs or true rotatable 3D geometry; do not claim a specific image model version unless the tool exposes and verifies it. Include interior views when relevant. Equipment fit and referee gesture review remain human gates. Playful off-ice extras can prompt gentle feedback but never count as required gear.
