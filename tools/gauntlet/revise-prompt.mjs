...
export function buildRevisePrompt({ scenario, ascii, node, concept, notes, errs = [], warns = [] }) {
  ...
  const prompt = [
    ...
    "- Make the SMALLEST fix. Do NOT redesign the board: keep the same stage/view/zone and the existing setup; move only the few actors you must. If the only fix would be a redesign, return \"edit\": {} and explain in \"change\".",
    "- NEVER place a defender in the passing or shooting lane of the correct answer — the correct read must stay genuinely open. Add pressure to the WRONG options, not the right one.",
