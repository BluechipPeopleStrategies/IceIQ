...
export function buildCoverageReport({ bank, ledger = loadLedger(), generatedAt = new Date().toISOString(), audit = null, bindings = null } = {}) {
  const inputBank = bank || readBankFiles().bank;
  const bindingPath=resolve(ROOT,'src/one-on-one/experimentalCurriculumBindings.json');
  const bindingRows=(bindings || (!bank&&existsSync(bindingPath)?JSON.parse(readFileSync(bindingPath,'utf8')):{rows:[]})).rows;
  const explicitBindings=inputBank.map(s=>{const b=bindingRows.find(r=>r.scenarioId===s.id);const valid=b&&b.scenarioVersion===s.version&&s.questions.every(q=>b.questionHashes?.[q.id]===questionContentHash(s,q));return {scenarioId:s.id,scenarioVersion:s.version,ageBand:s.ageBand,title:s.title,questions:s.questions.length,conceptIds:valid?b.conceptIds:[],rationale:valid?b.rationale:'Missing or stale explicit binding',status:valid?b.status:'needs-mapping-review'};});
...
</style></head><body><header><div class="shell"><h1>Curriculum coverage map</h1><p>Current composed experimental bank inventory: volume, delivery formats, geometry reuse, metadata signals and reviewable backlog candidates. Counts describe what exists; they do not certify mastery, quality or curriculum admission.</p></div></header><main class="shell"><p class="panel"><a href="/?arena=experimental&insights=curriculum#practice-arena">Open coverage with local attempts and flags</a> · <a href="../calibration/index.html">Review four calibration drafts</a></p><div id="cards" class="cards"></div><div class="toolbar"><label>Age<select id="age"><option value="">All ages</option></select></label><label>Topic<select id="topic"><option value="">All topics</option></select></label><label>Search metadata<input id="search" type="search" placeholder="topic, family, tag or objective"></label><span class="muted">Data: <a class="links" href="coverage.json">coverage.json</a> · <a class="links" href="README.md">README</a></span></div><nav id="tabs" class="tabs" aria-label="Coverage views"></nav><section id="content" class="panel"></section><p class="foot">Generated by <code>tools/build-curriculum-coverage.mjs</code>. The ledger supplies the six domains and age-depth nodes; current scenario metadata is retained as the evidence layer.</p></main><script id="coverage-data" type="application/json">${embedded}</script><script>
const DATA=JSON.parse(document.getElementById('coverage-data').textContent),ages=DATA.meta.ages,tabs=[['overview','Overview'],['age','Age'],['topic','Topic'],['family','Family'],['format','Formats'],['bindings','Concept bindings'],['domain','Keyword signals'],['source','Sources'],['backlog','Backlog']];let view='overview';const state={age:'',topic:'',search:''};
...
function render(){renderCards();document.querySelectorAll('#tabs button').forEach(b=>b.classList.toggle('active',b.dataset.view===view));const c=document.getElementById('content'),ss=filteredScenarios();if(view==='overview'){c.innerHTML='<h2>Observed shape</h2><p class="note">The bank contains '+DATA.overview.questions+' questions over '+DATA.overview.uniqueOpeningGeometry+' unique opening geometries. Several prompts can share one scene; that ratio is a production inventory measure.</p>'+renderHeat()+'<h2 style="margin-top:20px">What answers are based on</h2><p>Read the picture: visible facts with a definite answer. Choose or explain a play: coaching suggestions where other choices may be defensible. These categories are not quality ratings.</p>'+table(['Basis','Questions'],Object.entries(DATA.overview.basisCounts).map(([k,n])=>'<tr><td>'+esc(k==='scene'?'Read the picture':k==='coaching'?'Choose or explain a play':k)+'</td><td class="num">'+n+'</td></tr>'))+'<h2 style="margin-top:20px">Review boundary</h2><ul>'+DATA.qualityReviewRubric.map(x=>'<li>'+esc(x)+'</li>').join('')+'</ul>'}
...
Generated ${report.meta.generatedAt} by tools/build-curriculum-coverage.mjs from tools/experimental-bank-files.mjs and the current composed bank.
