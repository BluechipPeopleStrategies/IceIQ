...
const laterRows=[...(later?.results|| (later?.questionId?[later]:[])),...['original-user-feedback-recheck.json','related-board-recheck.json','calibration-final-recheck.json','packets-02-06-final-recheck.json'].flatMap(file=>fs.existsSync(`${dir}/followup/${file}`)?read(`followup/${file}`).coverage||[]:[])].filter(r=>originalIds.has(r.questionId));
