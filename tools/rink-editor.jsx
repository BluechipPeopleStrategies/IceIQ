      ...
      {mode === 'edit' && (
        <>
          ...
          <LabelToggles
            ...
            onToggle={(keys, hide) => updateScene(prev => {
...
// Per-scene toggles for which zone labels ("Corner", "Point", etc.) show up.
// Hidden labels are stored in scene.hiddenLabels (array of LABEL_KEYS keys).
function LabelToggles({ hiddenLabels, onToggle }) {
  const hidden = new Set(hiddenLabels || []);
  // Dedupe the left/right "Corner" & "Point" so the UI shows one chip per name
  // but toggling hides both sides at once (kids don't care which corner label).
  const groups = [
  ...
  return (
    <div style={styles.labelToggles}>
      <span style={styles.labelTogglesLabel}>Labels:</span>
      {groups.map(g => {
        const isHidden = g.keys.every(k => hidden.has(k));
        return (
          <button key={g.keys[0]}
                  onClick={() => onToggle(g.keys, !isHidden)}
