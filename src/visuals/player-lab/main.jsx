...
export function sampleLabScene(time,mode='forward'){
 ...
 return {time:t,actors:[
  {id:'N1',label:'7',number:7,team:'home',x:12+vx*t,y:-2,facing:mode==='rim'?Math.min(.7,t*.25):0,vx,vy:0,motion:{mode:moving?'skate':'glide',...(mode==='rim'?{action:{type:'rim-pickup-preparation',phases}}:{})}},
  {id:'G1',label:'4',number:4,team:'away',x:21-t*.8,y:1,facing:Math.PI,vx:-.8,vy:0,motion:{mode:'glide'}},
  {id:'N2',label:'11',number:11,team:'home',x:20,y:5,facing:0,vx:0,vy:0,motion:{mode:'ready'}},
  {id:'G2',label:'Goalie',number:31,team:'away',role:'goalie',x:27,y:0,facing:Math.PI,vx:0,vy:0,motion:{mode:'ready'}}
...
function App(){
 ...
 <div className="lab-options"><label>Model finish <select aria-label="Model finish" value={finish} onChange={e=>setFinish(e.target.value)}><option value="rounded-study">Rounded study</option><option value="integration-candidate">Existing candidate</option></select></label><label>Presentation <select aria-label="Player presentation" value={presentation} onChange={e=>setPresentation(e.target.value)}><option value="characters">Characters</option><option value="tactical">Tactical X/O</option></select></label><label>Player age <select aria-label="Player age" value={age} onChange={e=>setAge(e.target.value)}><option>U7</option><option>U11</option><option>U18</option></select></label><label>Movement example <select aria-label="Movement example" value={mode} onChange={e=>{setMode(e.target.value);setTime(0);setPlaying(false);}}><option value="forward">Forward skating</option><option value="backward">Backward defending</option><option value="glide">Glide</option><option value="rim">Rim preparation pose study</option></select></label><label>Question starting view <select aria-label="Question starting view" value={question} onChange={e=>changeQuestion(e.target.value)}><option value="broadcast">Broadcast</option><option value="overhead">Overhead</option><option value="side">External perspective</option><option value="eyes">Navy 7 player eyes</option><option value="defender">Gold 4 player eyes</option>{saved&&<option value="saved">Saved perspective</option>}</select></label></div>
