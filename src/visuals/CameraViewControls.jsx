...
export default function CameraViewControls({ actors = [], observerId, onObserverChange, preset, onPresetChange, adjusting = false, onAdjustingChange, panMode = false, onPanModeChange, onReset, onCameraCommand, presetOptions = CAMERA_VIEW_OPTIONS, disabled = false, instruction = '' }) {
  ...
  return <div className="camera-view-controls">
    ...
    {adjusting ? <div className="cvc-adjustment-panel">
      {onCameraCommand && <div className="cvc-direct-controls">
        {preset!=='first-person'&&<div role="group" aria-label="Scenario size"><span className="cvc-control-label">Zoom</span><div>{commandButton('Zoom out', '−', 'zoom', -1)}{commandButton('Zoom in', '+', 'zoom', 1)}</div></div>}
        ...
        {preset!=='first-person'&&<div role="group" aria-label="Scenario position"><span className="cvc-control-label">Move rink</span><div>{commandButton('Move left', '←', 'pan-x', -1)}{commandButton('Move right', '→', 'pan-x', 1)}{commandButton('Move up', '↑', 'pan-y', 1)}{commandButton('Move down', '↓', 'pan-y', -1)}</div></div>}
      </div>}
      <div className="cvc-adjustment">{preset!=='first-person'&&<div className="cvc-adjustment-modes" role="group" aria-label="Camera adjustment type"><button type="button" disabled={disabled} aria-label="Turn view" aria-pressed={!panMode} onClick={() => { if (!disabled) onPanModeChange(false); }}>Turn</button><button type="button" disabled={disabled} aria-label="Move view" aria-pressed={panMode} onClick={() => { if (!disabled) onPanModeChange(true); }}>Move</button></div>}<p role="status">{preset==='first-person'?'Drag or use arrow keys to look around from this player. Finish adjusting to answer.':<>Drag to {panMode ? 'move the view' : 'turn the view'}. Pinch or scroll to zoom. Arrow keys also work. Finish adjusting to answer.</>}</p></div>
    </div>
