...
const explicitNumber = actor => {
  const number = actor?.jerseyNumber ?? actor?.number;
  return /^(?:\d{1,2})$/.test(String(number)) ? String(number) : null;
};
export function compactActorLabel(actor) {
  ...
  if (explicitNumber(actor) !== null) return explicitNumber(actor);
  const namedTeam = /^(?:Navy|Gold)\s*(\d+)$/i.exec(actor.label || '');
  if (namedTeam) return namedTeam[1];
  if (['home','away'].includes(actor.team) && /^[HANG]\d+$/.test(actor.label || '')) return actor.label.slice(1);
...
export function actorDisplayName(actor) {
  ...
  if (label === 'G') return actor?.team === 'home' ? 'Navy goalie' : actor?.team === 'away' ? 'Gold goalie' : 'Goalie';
