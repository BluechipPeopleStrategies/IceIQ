...
// SVG PRIMITIVES
// ─────────────────────────────────────────────────────────────────────────────
function RinkBackground({ lineOpacity = 0.45, showHomePlate, showZoneOutlines }) {
  ...
  return (
    <>
      ...
      {/* Zone labels — kept at FULL opacity so coaches/kids can read them
          even when the rink lines are dim. No "half-wall" label. */}
      <ZoneLabel x={RINK_LEFT + 55}  y={GOAL_LINE_Y - 58} text="Corner" />
      <ZoneLabel x={RINK_RIGHT - 55} y={GOAL_LINE_Y - 58} text="Corner" />
      <ZoneLabel x={RINK_CENTER_X}   y={RINK_TOP - 8}     text="Behind the net" />
      <ZoneLabel x={RINK_CENTER_X}   y={FACEOFF_Y - 30}   text="Slot" />
      <ZoneLabel x={RINK_CENTER_X}   y={FACEOFF_Y + 80}   text="High slot" />
      <ZoneLabel x={L_FACEOFF_X - 56} y={BLUE_LINE_Y - 24} text="Point" />
      <ZoneLabel x={R_FACEOFF_X + 56} y={BLUE_LINE_Y - 24} text="Point" />
...
function Goalie() {
  const cx = RINK_CENTER_X;
  const cy = GOAL_LINE_Y + 16;
  return (
    <g pointerEvents="none">
    ...
    <div className={`rnk-wrap ${className}`} style={styles.wrap}>
      ...
      <div style={styles.canvasWrap}>
        <svg ref={svgRef}
          ...
          {/* Faint center-ice IceIQ brand mark */}
          <IceIQBrand variant="ghost" />
          ...
          {/* Corner IceIQ wordmark */}
          <IceIQBrand variant="corner" />
        </svg>
      </div>
