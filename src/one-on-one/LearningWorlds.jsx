...
import { getLearningWorlds, learningActivitiesForAge, missionAvailability } from './learningWorldsCore.js';
    ...
    <section id={regionId} ref={detailRef} className="lw-detail" style={artStyle(world)} aria-labelledby={`${regionId}-title`}>
      ...
      {mission ? <div className="lw-mission-layout">
        ...
        <article className="lw-mission" id={`${regionId}-mission`} aria-labelledby={`${regionId}-mission-title`}>
          ...
          {['study', 'loading', 'unknown'].includes(availability) && <button type="button" className="lw-secondary" onClick={() => navigate({ tab: 'learn', learn: availability === 'unknown' || ['U15', 'U18'].includes(band) ? 'library' : 'discover', ...(availability === 'unknown' || ['U15', 'U18'].includes(band) ? { conceptId: mission.conceptId } : {}) })}>{availability === 'unknown' || ['U15', 'U18'].includes(band) ? 'Open lesson library' : 'Explore the rink'} <span aria-hidden="true">→</span></button>}
        </article>
