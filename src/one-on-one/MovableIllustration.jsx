...
export default function MovableIllustration({children,label,onTap,allowPan=true}){
  ...
  return <div className="mi-root">
    <div className="mi-viewport" tabIndex={0} role="group" aria-label={`${label}. Drag to move; arrow keys move the view. Use zoom and reset below.`}
      onPointerDown={e=>{if(!allowPan||e.button!==0||e.target.closest('[role="button"],button,a'))return;const r=e.currentTarget.getBoundingClientRect();drag.current={x:e.clientX,y:e.clientY,w:r.width,h:r.height,moved:false};suppressClick.current=false;e.currentTarget.setPointerCapture(e.pointerId);}}
