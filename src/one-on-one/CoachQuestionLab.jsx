...
export function QuestionBoard({ draft, snapshotState, title, ageBand, startingView, questionId, selected, focusActorId, onSelect, onMove, editableTeam, allowedActorIds, selectableIds = [], onActorAnswer, onAvailabilityChange, view = 'full', young = false, showFacing = !young, ghostDraft, framingDrafts = [], initialFraming = 'closer', inspectable = false, sceneView = false }) {
  ...
  return <figure className="cq-board"><figcaption><strong>{title}</strong><span>{onMove ? sceneView ? 'Select a player · tap ice · coordinate controls' : 'Drag · select and tap ice · arrow keys' : 'Authored snapshot'}</span></figcaption>
    {sceneView ? <ScenarioRinkView ageBand={ageBand ?? (young ? "U7" : undefined)} startingView={startingView} questionId={questionId} state={sceneState} focusActorId={focusActorId} title={title} onAvailabilityChange={reportAvailability}
