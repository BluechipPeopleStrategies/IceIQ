...
function QuestionBoard({ draft, title, selected, onSelect, onMove, editableTeam, view = 'full', young = false, showFacing = !young, ghostDraft, inspectable = false }) {
  ...
  return <figure className="cq-board"><figcaption><strong>{title}</strong><span>{onMove ? 'Drag · select and tap ice · arrow keys' : 'Authored snapshot'}</span></figcaption>
    <svg ref={svg} viewBox={half ? '-1.5 -14.5 34 29' : '-32 -14.5 64 29'} role={onMove ? 'group' : 'img'} aria-label={title}
      ...
      <defs>
        <linearGradient id={`cq-ice-${instance}`} x1="0" x2="1" y1="0" y2="1"><stop stopColor="#fbfdff" /><stop offset=".52" stopColor="#eaf1f5" /><stop offset="1" stopColor="#cddde5" /></linearGradient>
      ...
      <g pointerEvents="none" filter={`url(#cq-depth-${instance})`}>
        <use href={`#cq-surface-${instance}`} fill="#f3f6f8" stroke="#213b51" strokeWidth=".78" />
        <use href={`#cq-surface-${instance}`} fill="none" stroke="#d0ae65" strokeWidth=".44" />
        <use href={`#cq-surface-${instance}`} fill={`url(#cq-ice-${instance})`} stroke="#edf4f7" strokeWidth=".14" />
      </g>
      ...
      {frame.actors.map(actor => <g key={actor.id} transform={`translate(${actor.x} ${actor.y})`} role={canMove(actor) ? 'button' : undefined} tabIndex={canMove(actor) ? 0 : undefined} aria-label={`${actor.label}, ${actor.team} ${actor.role}${canMove(actor) ? '. Arrow keys move this player.' : ''}`} onFocus={() => canMove(actor) && onSelect?.(actor.id)} onKeyDown={event => keyboard(event, actor)} onPointerDown={event => { event.stopPropagation(); begin(event, actor); }} className={canMove(actor) ? 'cq-movable' : ''}>
        ...
        <HockeyPlayerArt radius={.84} team={actor.team} goalie={actor.role === 'goalie'} facing={showFacing ? actor.facing * 180 / Math.PI : null} />
    ...
    </svg><div className="cq-legend"><span><i className="cq-home-mark" />Home · navy</span><span><i className="cq-away-mark" />Away · gold</span><span>● Puck</span>{ghostDraft && <span>◌ Dashed rings · coach reference</span>}</div>
    {inspectable && <BoardInspection title={title} renderBoard={() => <QuestionBoard draft={draft} title={title} selected={selected} editableTeam={editableTeam} view={view} young={young} showFacing={showFacing} ghostDraft={ghostDraft} />} />}
  </figure>;
}
