...
export function CurriculumBoard({ visual, title, ageBand, startingView, questionId, inspectable = false, sceneView = false, playing = false, time = 0, onAvailabilityChange }) {
  ...
  return <figure className="gc-board">
    ...
    {sceneView ? <ScenarioRinkView state={sceneState} title={`${title}. ${visual.caption}`} fallback={tacticalBoard}
      ageBand={ageBand} startingView={startingView} questionId={questionId} playing={playing} time={time} onAvailabilityChange={onAvailabilityChange}
    ...
    {inspectable && <BoardInspection title={title} renderBoard={() => <CurriculumBoard sceneView visual={visual} title={title} ageBand={ageBand} startingView={startingView} questionId={questionId} />} />}
  </figure>;
}
...
export function CurriculumSession({ playerId, ageBand, initialLessonId }) {
  ...
  return <section className="gc-root" aria-label="Guided hockey curriculum">
    ...
    <article className="gc-workspace">
      ...
      {placingPlayer ? <CurriculumPositionExercise question={question} playerId={playerId} ageBand={age} startingView={question.startingView} /> : <div className="gc-play-area"><div><CurriculumBoard key={boardIdentity} ageBand={age} startingView={question.startingView} questionId={boardIdentity} sceneView visual={{ ...motion.visual, caption: copy.visualCaption }} title={lesson.title} playing={motion.playing && sceneAvailable} time={motion.time} onAvailabilityChange={onAvailabilityChange} />
