...
export default function FoundationFlow({playerId,ageBand,onBack}){
  const age=normalizeAge(ageBand);
  if(!playerId||!['U7','U9','U11'].includes(age))return <section className="ff-root"><h1>Choose U7, U9 or U11 to explore foundations.</h1><button onClick={onBack}>Back to worlds</button></section>;
  return <Flow key={`${playerId}:${age}`} scope={{playerId,ageBand:age}} onBack={onBack}/>;
}
function Flow({scope,onBack}){
  ...
  function pack(id,remove=false){const item=content.gear.find(g=>g[0]===id);if(!item)return;update(current=>({...current,packed:remove?current.packed.filter(x=>x!==id):[...new Set([...current.packed,id])]}));setGearNote((remove?'Taken out. ':'')+item[2]);}
  ...
  return <section className="ff-root" aria-label="Foundations learning flow">
    <header className="ff-header"><button onClick={onBack}>Back to worlds</button><span>{scope.ageBand} · Frozen Trails</span></header>
    ...
    {flow.stage<=1&&<>
      ...
      <div className="ff-rink" ref={rinkRef} onClick={chooseRink} onKeyDown={e=>{if((e.key==='Enter'||e.key===' ')&&e.target.matches('g[role="button"]')){e.preventDefault();chooseRink(e);}}} dangerouslySetInnerHTML={{__html:foundationRink({mode:flow.stage===0?'landmarks':map?'heat':'lineup',spot,role,phase,turnover})}}/>
    ...
    {(flow.stage===3||flow.stage===4)&&<>
      ...
      <div className="ff-signal-layout"><div className="ff-signal-art" dangerouslySetInnerHTML={{__html:refArt(currentSignal)}}/><div>
