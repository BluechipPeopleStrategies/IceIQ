...
export default function PracticeLibrary({playerId='practice-preview',ageBand='U11',initialConcept=''}) {
  const [bank,setBank]=useState(null),[error,setError]=useState('');
  const [age,setAge]=useState(()=>LEVELS.find(value=>value.split(' ')[0]===String(ageBand).split(' ')[0])||'U11 / Atom'),[concept,setConcept]=useState(initialConcept),[type,setType]=useState(''),[search,setSearch]=useState(''),[selected,setSelected]=useState(null),[attempt,setAttempt]=useState(0);
  const progressKey=`rinkreads_practice_lessons_v1:${playerId}`;
  const [progress,setProgress]=useState(()=>readProgress(progressKey));
  const masteryKey=masteryStorageKey(playerId);
  ...
  const selectedProgress=selectedMastery?masteryProgress(mastery,selectedMastery,{catalog:masteryCatalog}):null;
  ...
  const records=Object.values(progress),points=records.reduce((s,r)=>s+(Number(r?.points)||0),0);
  function credit(correct){
    if(!selected)return;
    setProgress(old=>{const next=creditLesson(old,selected.key,correct);try{localStorage.setItem(progressKey,JSON.stringify(next));}catch{setNotice('Progress is available for this visit; browser storage is unavailable.');}return next;});
    const now=new Date().toISOString();
    const saved=savePracticeEvidence({playerId,attempt:{...selectedMastery,correct},now});
    if(saved.status==='saved') { setMastery(saved.ledger); setNotice('Practice evidence saved on this device.'); }
    else if(saved.status==='unavailable') setNotice('Your answer was checked, but practice evidence could not be saved. Existing history has been kept.');
  }
  return <section className="pf-library">
    <div className="pf-section-title"><div><p className="oo-eyebrow">YOUR RINKREADS CURRICULUM</p><h1>Learn the read.<br/><em>Then put it on the ice.</em></h1></div><div className="pf-score"><strong>{masteryTotals.points.toLocaleString()}</strong><span>{age.split(' ')[0]} MASTERY POINTS · {masteryTotals.mastered} GROUPS</span><small>{points.toLocaleString()} historical practice points preserved<br/>Saved on this device</small></div></div>
    ...
    <div className="pf-library-grid"><aside className="pf-lesson-list"><p className="oo-eyebrow">{matches.length} LESSONS</p>{error&&<p role="alert">{error}</p>}{!bank&&!error&&<p>Loading your lessons…</p>}{bank&&!matches.length&&<p>No lessons match these filters. Try another age or format.</p>}{matches.map(i=><button className={selected?.key===i.key?'active':''} key={i.key} onClick={()=>{setSelected(i);setAttempt(0)}}><small>{TYPE_LABELS[i.type]||i.type}{progress[i.key]?' · Practised':''}</small><strong>{i.title}</strong><span>{i.concept.replaceAll('-',' ')}</span></button>)}</aside>
      <article className="pf-lesson-stage">{selected?<><div className="pf-lesson-toolbar"><button onClick={()=>setAttempt(a=>a+1)}>Try again</button></div>
      <SpacedMasteryProgress progress={selectedProgress} eligible={selectedMastery.eligible}/>
      {selected.type==='animated-play'?<AnimatedPlay key={`${selected.key}:${attempt}`} play={selected.source} ageBand={age.split(' ')[0]}/>:<SourceQuestion playerId={playerId} key={`${selected.key}:${attempt}`} item={selected} onCredit={credit}/>}
      </>:<div className="pf-library-empty"><span>↖</span><h2>Pick a concept.<br/>Find your next read.</h2><p>Your multiple-choice questions, true/false questions, scenarios and animated plays are all here. Choose a lesson to begin.</p></div>}{notice&&<p role="status">{notice}</p>}</article></div>
  </section>;
}
