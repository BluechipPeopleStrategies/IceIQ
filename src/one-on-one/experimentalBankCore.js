...
export function validateExperimentalBank(bank, expectedCounts) {
 ...
 for(const s of bank){
  ...
  if(!Array.isArray(s.questions)||s.questions.length<6||s.questions.length>8)fail('six to eight questions required');
...
export function catalogCsv(bank){
 ...
 for(const s of bank)for(const q of s.questions)rows.push([s.id,s.version,s.ageBand,s.title,s.family,s.topic,q.id,q.type,q.prompt,q.basis,q.type==='position'?JSON.stringify(q.reference):q.answer?.map(id=>q.options.find(o=>o.id===id)?.text).join(' | ')||'',q.explanation,s.sources.map(x=>`${x.title}: ${x.url} (${x.section})`).join(' | '),'Experimental; AI review recorded; human coach approval pending']);
