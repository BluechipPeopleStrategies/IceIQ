...
function ScenarioQuestions({scenario:s,record,onRecord,onFlag,onMetric}){
 ...
 const rubric=import.meta.env.DEV?placementPack.rubrics.find(r=>r.questionId===q.id):null;
 const areaCells=useMemo(()=>isPosition&&rubric?samplePlacementAreas({scene:s.setup,rubric,questionId:q.id,scenarioVersion:s.version,sourceSignature:placementSource(s,q)}):[],[s,q,rubric,isPosition]);
 const displayScene=showReview&&showAreas?{...scene,overlays:{...scene.overlays,cells:areaCells}}:scene;
...
export default function ExperimentalPractice({playerId='practice-preview',bank:suppliedBank=EXPERIMENTAL_BANK,initialAge=''}){
 ...
 const pilot=import.meta.env.DEV&&calibrationId==='coaching-pilot-2026-09-06';
 ...
 const metric=useCallback((event,s,q,extra={})=>{
  ...
  const meta={...identity,questionId:q.id,basis:q.basis,questionType:q.type,ageBand:s.ageBand,topic:s.topic,family:s.family,questionOptionIds:q.options?.map(o=>o.id),...extra};
