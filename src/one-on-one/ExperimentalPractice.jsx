 ...
 return <section className="ep-scenario" aria-label={s.title}>
  ...
  <div className="ep-workspace">
   <div className="ep-rink-column"><div className="ep-view-choice"><span>Navy attacks the right net</span><button type="button" aria-pressed={board} onClick={()=>setBoard(v=>!v)}>{board?'Open 3D rink':'Use overhead board'}</button></div>
    {board||!availability?<ExperimentalBoard scene={scene} onPoint={isPosition?move:null} reference={result&&isPosition?q.reference:null}/>:<ScenarioRinkView state={scene} title={s.title} ageBand={s.ageBand} startingView={q.startingView ?? s.startingView} questionId={`${s.id}:${s.version}:${q.id}`} focusKey={`${s.id}:${s.version}:${q.id}`} focusPoints={isPosition&&q.reference?[q.reference]:[]} focusActorId={isPosition?q.actorId:s.focusActorId} selectedActorId={isPosition?q.actorId:null} editableIds={isPosition?[q.actorId]:[]} onSelect={()=>{}} onMove={(_,point)=>move(point)} onAvailabilityChange={setAvailability} teamLabels={{home:'Navy',away:'Gold'}} onViewUsage={cameraAction=>onMetric('camera',s,q,{cameraAction})}/>}
