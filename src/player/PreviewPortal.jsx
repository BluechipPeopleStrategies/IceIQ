...
import {loadFlow,saveFlow} from '../one-on-one/foundationFlowCore.js';
...
const AGE_KEY='rinkreads_public_preview_age_v1';
function remembered(){try{return sessionStorage.getItem(SESSION_KEY)==='open';}catch{return false;}}
function rememberedAge(){try{return previewAge(sessionStorage.getItem(AGE_KEY));}catch{return 'U7';}}
export default function PreviewPortal(){
  const [open,setOpen]=useState(remembered),[code,setCode]=useState(''),[error,setError]=useState(''),[age,setAge]=useState(rememberedAge),[activity,setActivity]=useState(false);
  const heading=useRef(null);
  const [entryStage,setEntryStage]=useState(null);
  function home(event){event?.preventDefault();setActivity(false);requestAnimationFrame(()=>{heading.current?.focus();window.scrollTo({top:0,behavior:'instant'});});}
  function chooseAge(value){const next=previewAge(value);setAge(next);try{sessionStorage.setItem(AGE_KEY,next);}catch{}setActivity(false);}
  function start(stage){setEntryStage(Number.isInteger(stage)?stage:null);if(Number.isInteger(stage)&&stage>=0&&stage<=5){try{const scope={playerId:previewPlayerId(age),ageBand:age};const {flow}=loadFlow(localStorage,scope);saveFlow(localStorage,scope,{...flow,stage});}catch{}}setActivity(true);}
  ...
  return <main className="pp-root">
    <header className="pp-nav"><a href="#" aria-label="RinkReads home" onClick={open?home:undefined}>RinkReads<span>.</span></a>{open?<button onClick={leave}>Leave preview</button>:import.meta.env.MODE==='preview'?<span>Player &amp; family preview</span>:<a href="#">Back to site</a>}</header>
    {!open?<section className="pp-gate"><p className="pp-eyebrow">YOUR FIRST SHIFT</p><h1>Step onto the ice.</h1><p>Enter the preview code you were given. Explore the rink, equipment and referee signals—no account needed.</p>
      ...
      <section className={`pp-intro${activity?' is-activity':''}`}><h1 ref={heading} tabIndex={-1}>{activity?'Explore the demo':'Take your first shift.'}</h1>{!activity&&<p>Early preview · learning content is still under review. Explore together, at your own pace.</p>}<label>Choose an age group <select value={age} onChange={e=>chooseAge(e.target.value)}><option>U7</option><option>U9</option><option>U11</option></select></label></section>
      ...
      <Suspense fallback={<p role="status">Getting the rink ready…</p>}>{activity?<FoundationFlow key={age} playerId={previewPlayerId(age)} ageBand={age} onBack={()=>home()} backLabel="Back to preview home" allowSkip initialStage={entryStage}/>:<PilotHome key={age} playerId={previewPlayerId(age)} ageBand={age} onStart={start} publicPreview/>}</Suspense>
