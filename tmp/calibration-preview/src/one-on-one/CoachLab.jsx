...
export default function CoachLab({sourcePlay,initialDraft,onDraftChange,onCreateQuestion,playerId='practice-preview'}) {
  ...
  useEffect(()=>{
    ...
    window.addEventListener('keyup',up);window.addEventListener('blur',stop);document.addEventListener('visibilitychange',hide);id=requestAnimationFrame(tick);
    return()=>{cancelAnimationFrame(id);window.removeEventListener('keyup',up);window.removeEventListener('blur',stop);document.removeEventListener('visibilitychange',hide)};
  },[stop]);
  ...
  function playLive(){stop();try{frameRef.current=createTeamGame(draftRef.current);setFrame(frameRef.current);liveRef.current=true;setLive(true);timeRef.current=0;setTime(0);runRef.current=true;setRunning(true);playSurface.current?.focus();}catch(e){setNotice(e.message)}}
  function resumeLive(){if(resumeCoachPlay({liveRef,frameRef,runRef,input:{keys,action,stick}})){setRunning(true);playSurface.current?.focus();}}
