...
try {
    ...
    $openArtTempRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath()).TrimEnd('\') + '\'
    $openArtCleanup = [IO.Path]::GetFullPath($tmp)
    if (-not $openArtCleanup.StartsWith($openArtTempRoot, [StringComparison]::OrdinalIgnoreCase) -or $openArtCleanup -eq $openArtTempRoot.TrimEnd('\')) { throw 'Unexpected cleanup path' }
    Remove-Item -LiteralPath $openArtCleanup -Recurse -Force -ErrorAction SilentlyContinue
}
