...
function SceneContents({ frame, frameRef, bounds, definition, targets, onTarget, moveActorId, focusActorId, onMove, onRoutePoint, onFirstAction, route, playing, cameraPreset, cameraAdjusting, cameraPanMode, cameraCommand, cameraResetToken }) {
  const { invalidate } = useThree();
  ...
  return <>
    ...
    <ScenarioCamera cameraDirect bounds={bounds} cameraPreset={cameraPreset} cameraAdjusting={cameraAdjusting} cameraPanMode={cameraPanMode} cameraCommand={cameraCommand} cameraResetToken={cameraResetToken} /><Arena openView /><Ice clearBoards hideZoneLines={definition.ui?.hideZoneLines === true || definition.ageBand === 'U9'} /><Goal />
    <PlannedRoute route={route} />
    {frame.actors.map(actor => <Skater showHeading={cameraPreset === 'overhead'} key={actor.id} frameRef={frameRef} actorKey={actor.id} colour={actor.team === 'home' ? '#0B1A33' : '#C9A24B'} number={JERSEY_NUMBERS[actor.id] || '8'} goalie={actor.role === 'goalie'} selected={actor.id === moveActorId} isLearner={isFocusedActor(actor, focusActorId)} />)}
    <Puck frameRef={frameRef} />
    {passReceiver && <Html center position={world(passReceiver, .9)} zIndexRange={[25, 15]} style={{ pointerEvents: answering ? 'auto' : 'none' }}><RinkActorAnswer actorId={passReceiver.id} onAnswer={() => onFirstAction('pass')} enabled={answering} className="rs-first-action-hit" aria-label={labelledActors ? `Pass to ${compactActorLabel(passReceiver)}` : 'Pass to your teammate'}><span className="rs-scene-sr-only">Pass</span></RinkActorAnswer></Html>}
    {passReceiver && <RinkActionCue action="pass" point={passReceiver} actors={frame.actors} puck={frame.puck} onAnswer={() => onFirstAction('pass')} enabled={answering} label={labelledActors ? `Select pass to ${compactActorLabel(passReceiver)}` : 'Select pass to your teammate'} />}
    {carrier && <RinkActionCue action="carry" point={carrier} actors={frame.actors} puck={frame.puck} offset={[-46, 0]} onAnswer={() => onFirstAction('carry')} enabled={answering} label="Carry the puck into space" />}
    {firstActions.includes('shoot') && <RinkGoalAnswer onAnswer={() => onFirstAction('shoot')} enabled={answering} />}
    {frame.actors.map(actor => <ActorChip key={actor.id} actor={actor} focused={isFocusedActor(actor, focusActorId)} labelledActors={labelledActors} movable={movable && actor.id === moveActorId} />)}
...
function ReadScene({ state, definition, playing = false, time = 0, supportPoint = null, route = null, wide = false, targets = [], onTarget, moveActorId = null, focusActorId, onMove, onRoutePoint, onFirstAction, onFailure, onReady, cameraPreset = 'broadcast', cameraAdjusting = false, cameraPanMode = false, cameraCommand = null, cameraResetToken = 0 }) {
  ...
  return <div className="rs-scene3d" role="group" aria-label={`${definition.ageBand} connected read. Three-dimensional rink. Navy players attack; gold players defend.`}>
    <Canvas orthographic frameloop="demand" aria-label={cameraAdjusting ? 'Adjust hockey camera. Drag or use arrow keys to rotate; pinch, scroll or use plus and minus to zoom.' : 'Hockey play with labelled players, puck and net'} style={{ touchAction: cameraAdjusting ? 'none' : 'pan-y' }} shadows={{ type: THREE.PCFShadowMap }} dpr={[1.5, 2]}
      ...
      onCreated={({ gl }) => { lossCleanup.current?.(); lossCleanup.current = watchWebglContextLoss(gl.domElement, fail); onReady?.(); }}>
      <SceneContents {...{ frame, frameRef, bounds, definition, targets, onTarget, moveActorId, focusActorId, onMove, onRoutePoint, onFirstAction, route, playing, cameraPreset, cameraAdjusting, cameraPanMode, cameraCommand, cameraResetToken }} />
    </Canvas>
  </div>;
}
