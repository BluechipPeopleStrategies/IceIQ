...
function Content({ frameRef, camera, onPlace, selectedActor, focusActorId, showGuides, roster, onSelect, axesRef, routePoints, onRoutePoint }) {
  ...
  return <>
    ...
    {roster ? roster.map((a,i)=><Skater key={a.id} frameRef={frameRef} actorKey={a.id} colour={a.team==='home'?'#0B1A33':'#C9A24B'} number={a.label||String(i+1)} goalie={a.role==='goalie'} selected={selectedActor===a.id} isLearner={isFocusedActor(a,focusActorId)}/>) : <><Skater frameRef={frameRef} actorKey="attacker" colour="#0B1A33" number="17" selected={selectedActor === 'attacker' || (!selectedActor && state?.setup?.role !== 'defender')} isLearner={focusActorId === undefined ? state?.setup?.role !== 'defender' : focusActorId === 'attacker'} />
    <Skater frameRef={frameRef} actorKey="defender" colour="#C9A24B" number="8" selected={selectedActor === 'defender' || (!selectedActor && state?.setup?.role === 'defender')} isLearner={focusActorId === undefined ? state?.setup?.role === 'defender' : focusActorId === 'defender'} />
