...
const folder=`${dir}/packets-15-16`;
...
write(`${folder}/application-receipt.json`,receipt);
write(`${dir}/repairs/claude-packets-15-16-repairs.json`,receipt);
const independent=JSON.parse(fs.readFileSync(receiptPath));
write(`${dir}/followup/packets-15-16-final-recheck.json`,{...independent,coverage:independent.questions.map(q=>({...q,contentHash:q.afterHash}))});
