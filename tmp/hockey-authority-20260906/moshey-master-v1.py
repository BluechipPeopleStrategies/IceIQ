...
EXPECTED_SRC_SHA = '6b47b45d9cac51f13bb6e0ddf82adcafd9c102f628a2fc026f8917cd13943138'
...
jersey_name = jersey_obj.name if jersey_obj else None
subdivision_applied = False
# Root verification: disconnected exported triangles cannot be safely subdivided.
if jersey_obj is not None:
    edge_counts = {}
    for poly in jersey_obj.data.polygons:
        for key in poly.edge_keys:
            key = tuple(sorted(key)); edge_counts[key] = edge_counts.get(key, 0) + 1
    shared_edges = sum(n > 1 for n in edge_counts.values())
    safe_subdivision = shared_edges > len(edge_counts) * .25
else:
    safe_subdivision = False
if jersey_obj is not None and safe_subdivision:
...
assert sha256(SRC) == src_sha, 'source changed during production'
log = {
    'candidate': 'moshey-master-v1',
    'source': str(SRC),
    'source_sha256': src_sha,
    'source_unmodified': sha256(SRC) == src_sha,
    'units': 'metres',
    'skeleton_preserved': {'bones': sum(len(a.data.bones) for a in v_arm), 'vertex_groups_per_mesh': 13},
    'asset_excludes_stage': True,
    'jersey_mesh_subdivided': jersey_name if subdivision_applied else None,
    'root_subdivision_topology_guard': 'No subdividing disconnected exported triangles',
    'modifications': [
        'Angle-based smooth shading (40 deg) on all merged meshes: removes faceting, keeps hard corners; no vertex/weight change.',
        ('Subdivision Surface level 1 on fabric' if subdivision_applied else 'Subdivision skipped: disconnected topology would fragment the cloth'),
