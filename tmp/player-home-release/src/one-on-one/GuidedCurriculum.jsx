...
function CurriculumSession({ playerId, ageBand, initialLessonId }) {
  ...
  return <section className="gc-root" aria-label="Guided hockey curriculum">
    <p className="gc-storage-notice">Draft guided practice · no mastery points. Eligible library questions build mastery across different questions and dates.</p>
    <header className="gc-header"><div><p className="gc-kicker">RINKREADS / YOUR NEXT READ</p><h1>See the ice.<br /><em>Make your choice.</em></h1><p className="gc-intro">Guided starters. Two quick reads in each lesson.<br />Take your time. Learn from the picture.</p></div><div className="gc-score"><strong>{total.points.toLocaleString()}</strong><span>HISTORICAL PRACTICE POINTS</span><small>{total.attempted} of {total.total} reads explored<br />Saved on this device</small></div></header>
    ...
    <nav className="gc-lesson-cards" aria-label={`${age} lessons`}>{lessons.map((item, index) => {
      ...
      return <button key={item.id} className={index === lessonIndex ? 'active' : ''} aria-current={index === lessonIndex ? 'step' : undefined} onClick={() => openLesson(index)}><span className="gc-card-top"><b>{String(index + 1).padStart(2, '0')}</b><i>{stats.mastered === 2 ? '✓' : STRANDS[item.curriculumStrand].icon}</i></span><strong>{item.title}</strong><span className="gc-card-bottom">{STRANDS[item.curriculumStrand].label}<b>{stats.mastered === 2 ? 'Previously completed' : `${stats.attempted} / 2 explored`}</b></span></button>;
    })}</nav>
    <article className="gc-workspace">
      ...
      <div className="gc-play-area"><CurriculumBoard inspectable sceneView visual={question.visual} title={lesson.title} /><div className="gc-question-panel">
        {complete ? <div className="gc-complete" role="status"><span className="gc-complete-icon">{current.mastered === 2 ? '★' : '↻'}</span><p className="gc-kicker">LESSON EXPLORED</p><h3 ref={promptRef} tabIndex={-1}>{current.mastered === 2 ? 'You found both reads.' : 'Keep building this read.'}</h3><p>{current.attempted} of 2 reads explored · {current.points} historical points</p><p className="gc-complete-tip">{lesson.teachingPoint}</p><button className="gc-primary" onClick={() => openLesson((lessonIndex + 1) % lessons.length)}>{lessonIndex === lessons.length - 1 ? 'Return to the first lesson' : 'Next lesson →'}</button><button className="gc-text-button" onClick={() => openLesson(lessonIndex)}>Review this lesson</button><small>These draft starters record practice only. Eligible library questions build mastery across days.</small></div> : <>
